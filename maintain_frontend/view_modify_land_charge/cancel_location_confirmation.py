...
from maintain_frontend.view_modify_land_charge.validation.location_confirmation_validator import \
    LocationConfirmationValidator
...
def register_routes(bp):
    bp.add_url_rule('/cancel-location-confirmation',
                    view_func=get_cancel_location_confirmation,
                    methods=['GET'])
    bp.add_url_rule('/cancel-location-confirmation',
                    view_func=post_cancel_location_confirmation,
                    methods=['POST'])


@requires_permission([Permissions.vary_llc])
def get_cancel_location_confirmation():
    ...
    current_app.logger.info("Displaying page 'cancel_location_confirmation.html'")

    return render_template('cancel_location_confirmation.html',
                           submit_url=url_for('cancel_land_charge.post_cancel_location_confirmation'),
                           charge_id=calc_display_id(g.session.add_charge_state.local_land_charge))


@requires_permission([Permissions.vary_llc])
def post_cancel_location_confirmation():
    current_app.logger.info('Endpoint called')
    ...
    validation_errors = LocationConfirmationValidator.validate(confirmation, 'cancel')

    if validation_errors.errors:
        current_app.logger.warning('Validation errors occurred')
        return render_template(
            'cancel_location_confirmation.html',
    ...
    current_app.logger.info(
        "Session charge updated - Redirecting back to cancel_land_charge with local_land_charge='{}'"
        .format(charge_display_id)
    )
    return redirect(url_for("cancel_land_charge.cancel_charge", charge_id=charge_display_id))
